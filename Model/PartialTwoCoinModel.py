...
class PartialTwoCoinModel:
    '''
    Simulation model for Game 2. 
    '''    
    def __init__(self, max_I1, max_I2, max_d1, max_d2, max_d = None):
    ...
    def simulate_tie_probability(self, m: int, n, d, d1 = 1, d2 = 1, n_trials: int = 1000000) -> float:
        tie_count = a_win_count = b_win_count = 0
        for _ in range(n_trials):
            a = m 
            b = n 
            while True:
                a_toss_1  = random.randint(0, 1) 
                ...
                if a <= 0 and b <= 0: 
                    ...
                    break
        return {
        "tie": tie_count / n_trials,
        "A_win": a_win_count / n_trials,
        "B_win": b_win_count / n_trials 
        }


    
    @lru_cache(None)
    def tie_prob_by_recurrence(self, m, n, d, d1=1, d2=1):
        if m <= 0 and n <= 0:
            return Fraction(1)
        if m <= 0 < n or n <= 0 < m:
            return Fraction(0)
        return (
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d , n - d1 - d2, d, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d , n - d1     , d, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d , n - d2     , d, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d , n          , d, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d1 - d2, d, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d2     , d, d1, d2) + 
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d1     , d, d1, d2) 
        )
    ...
    def _coeff_B(self, I1, I2, d, d1, d2): 
        result = Fraction(0) 
        if I1 % d == 0:
            for i, j, k in self._nonnegative_solutions(d1, d2, I2): 
                result += self._multinomial(i, j, k) * self._D(I1 // d, i + j + k) 
        return result

    def tie_prob_by_gf(self, m, n, d, d1, d2, prob = PROB):
        total = Fraction(0)
        alpha_steps = (d,)
        ...
        for a, b in step_pairs:
            for M in range(1, min(a, m) + 1):
                for N in range(1, min(b, n) + 1):
                    if (m - M, n - N, d, d1, d2) in self.dict_coeff_B: 
                        total += self.dict_coeff_B[(m - M, n - N, d, d1, d2)] 
                    else:
                        coeff = self._coeff_B(m - M, n - N, d, d1, d2) 
                        total += coeff 
                        self.dict_coeff_B[(m - M, n - N, d, d1, d2)] = coeff 
        return prob * total
