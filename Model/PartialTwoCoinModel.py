...
class PartialTwoCoinModel:
    ...
    def simulate_tie_probability(self, m: int, n,  d1 = 1, d2 = 1, n_trials: int = 1000000) -> float:
        tie_count = 0
    ...
    def tie_prob_by_recurrence(self, m, n, d1=1, d2=1):
        ...
        if m <= 0 < n or n <= 0 < m:
            return Fraction(0)
        return (
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d1, n - d1 - d2, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d1, n - d2     , d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d1 - d2, d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d1, n - d1     , d1, d2) +
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d2     , d1, d2) + 
            Fraction(1, 7) * self.tie_prob_by_recurrence(m     , n - d1     , d1, d2) + 
            Fraction(1, 7) * self.tie_prob_by_recurrence(m - d1, n          , d1, d2) 
        )
    ...
    def _coeff_B(self, I1, I2, d1, d2): 
        ...
        if I1 % d1 == 0:
            for i, j, k in self._nonnegative_solutions(d1, d2, I2): 
                result += self._multinomial(i, j, k) * self._D(I1 // d1, i + j + k) 
        return result
